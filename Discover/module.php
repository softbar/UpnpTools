...
class UPnPDiscover extends IPSModule {
	public function Create(){
		...
		$this->RegisterPropertyInteger('DiscoverTimeout', 5);
		$this->RegisterAttributeString('DiscoverList', '');//file_exists(RPCTOOLS.'/discover/list.json')?file_get_contents(RPCTOOLS.'/discover/list.json'):'');
	}
	public function GetConfigurationForm(){
		...
		$ips=[];
		if(count($values)){
 			$f['elements'][0]['visible']=false;
 			$f['elements'][1]['visible']=false;
		}
		if($v=Sys_GetNetworkInfo())foreach($v as $n)$ips[]=['label'=>$n['IP'],'value'=>$n['IP']];
		$f['elements'][2]['items'][0]['options']=$ips;
	...
	public function RequestAction($Ident, $Value){
		if($Ident=="DISCOVER"){
			...
			$l=$this->DiscoverNetwork($UpdateProgress);
// 			$this->WriteAttributeString('DiscoverList', json_encode($l));
	...
	public function ApplyChanges(){
		parent::ApplyChanges();
		if(!$this->ReadPropertyBoolean('EnableCache'))$this->DiscoverCleanUpCache();
	}
	public function Reset(){
		$this->DiscoverCleanUpCache();
		$this->WriteAttributeString('DiscoverList', '');
		if(method_exists($this, 'UpdateFormField'))
			$this->UpdateFormField('DiscoverList', 'values', '[]');
	}
		...
		foreach($newRootIndex as $newItem){
			...
			if(!$found){
				...
				foreach($newSubItems as $item){
					...
					if(!$found){
						$item['id']=$nextID++;
	// 					array_splice($myList, $myIndex,null,$item);
						$myList[]=$item;
					}
				}
			}
	...
	protected function DiscoverNetwork($UpdateProgress=false){
		if($UpdateProgress){
		...
		$timeout=$this->ReadPropertyInteger('DiscoverTimeout');$retrys=1; 
		...
		for($j=0;$j<$retrys;$j++){
			...
			socket_bind($socket, $this->ReadPropertyString('BindIp'));
...
// 		$find= array_values($find);
		...
		foreach ($find as $found){
			...
			$this->UpdateFormField('ProgressBar', 'caption', substr($d[D_NAME],0,25).'...');
 			$item=[
				'id'=>$id,
				'address'=>$d[D_HOST],
				'name'=>$d[D_INFO],
	...
	// 					echo "Send Event : $var->name\n";	
		...
		$importFunctions=function($xml, &$curService,$sn)use(&$device,$options,$addStateVariable,$getStateVar,$getFunctionProp,$isValidFilter,$importEvents){
			...
			if($funcs && count($funcs)>0 && ($options&DISCOVER_EVENTS)>0)$importEvents($xml,$curService);
