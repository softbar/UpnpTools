...
class MediaDevice extends UPnPPropModule {
	...
	public function Create(){
		parent::Create();
		$this->RegisterPropertyInteger('VolumeLimit', 25);
	...
	public function GetConfigurationForm(){
		...
		$f->elements[2]->items[1]->options=$ips;
		
		$events=$this->ReadPropertyBoolean('EnableEvents');
		if($status!=102)$f->elements[0]->expanded=true;
		if($events){
			$f->actions[0]->values=$this->LoadEventList();
		}
		else unset($f->actions);
...
// 		return;
	...
	private function _writeValue(string $Ident, string $Value){
		...
		switch($Ident){
			case 'VOLUME'	:{
				if(!($limit=$this->ReadPropertyInteger('VolumeLimit'))|| $Value<= $limit)
					$ok=$this->CallApi('RenderingControl.SetVolume', [0,null,$Value=(int)$Value]);
				else $ok=false;
				break;
			}
